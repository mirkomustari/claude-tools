...
| **`ctxlint`** (npm, zero dipendenze, 41 regole sui file di contesto contro il codice) | **Adottato come audit manuale, non come gate** | Provato su djmeta: misura 2 769 token per `CLAUDE.md` e pesa le sezioni, trova un percorso impreciso (`djmeta/api.py` per `src/djmeta/api.py`), nota tre divieti «inviolabili» senza hook dietro. Ma prende per percorsi anche glob e frasi (`Linux/WSL2**`, `docs/Mxx_RISULTATI.md`) e cerca `package.json` in un progetto Python: come gate farebbe rumore. Un `npx` quando serve la mappa dei token; è nella skill come passo opzionale. *Sera dello stesso giorno*: **v0.9.10 bloccata** nella skill, nella rule di djmeta e in un job CI advisory; su djmeta segnala 6 «errori», tutti falsi positivi (`req/s`, `numpy/scipy/soundfile`, `/mnt/s/`, «index/body»), e due divieti senza hook dietro. Ha `--fix` per i percorsi rotti, una GitHub Action e un server MCP; la 0.10.1 dà lo stesso output |
...
- **2026-09-07**, terza passata — la ricerca sulla dieta: `ctxbudget.py`, la
  ...
  CI di djmeta.
- **2026-09-07**, quarta passata — la dieta di `CLAUDE.md` aveva lasciato
  quattordici citazioni «§5.1 / §5.2 / §5.3 / §4.5» orfane fra documenti,
  codice e test: in djmeta nasce il test che risolve ogni `CLAUDE.md §N` e ogni
  percorso citato nei documenti vivi; `mdcheck --hook` controlla anche i `.md`
  che un comando `Bash`/`PowerShell` nomina (il matcher era `Edit|Write`, e in
  auto mode le modifiche passano da `sed`); la skill `docs-drift` copre a
  chiusura milestone la metà di giudizio; `ctxlint` bloccato alla 0.9.10 come
  job CI advisory.
